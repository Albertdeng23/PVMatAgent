...
PVmatAgent 是一个专为**光伏材料（尤其是无铅/卤化物钙钛矿）研发**设计的智能科学助手（AI for Science Agent）。系统创新性地采用了**“神经-符号（Neuro-symbolic）”**协同范式，以云端大语言模型（LLMs）为逻辑中枢，通过原生函数调用（Tool Calling）架构，无缝调度底层的 13 项包含物理计算、深度学习预测、化学数据库与工程评估的“专家工具”。
...
### ⚛️ 模块六：第一性原理计算专家 (Ab Initio Calculation) - 新增
14. **`tool_vasp_connect` (VASP服务器连接)**：通过SSH连接到远程超算服务器，建立VASP计算环境。
15. **`tool_vasp_prepare_and_submit` (VASP计算提交)**：⚠️ **极度谨慎使用** - 准备输入文件(POSCAR, INCAR, KPOINTS)并提交第一性原理DFT计算。
16. **`tool_vasp_check_status` (VASP状态检查)**：查询计算任务进度（运行中/已完成/失败）。
17. **`tool_vasp_download_results` (结果下载)**：下载计算完成的OUTCAR、vasprun.xml等结果文件。
18. **`tool_vasp_parse_bandstructure` (能带解析)**：从EIGENVAL文件提取能带结构和带隙数据。

**⚠️ VASP工具使用警告**：
- VASP计算极其耗时（通常数小时到数天），消耗大量计算资源
- **必须遵循"先快后慢"原则**：优先使用CHGNet/MEGNet进行快速筛选
- 仅在以下情况使用：用户明确要求、机器学习结果存疑需验证、需要高精度能带数据用于发表论文
- Agent已被Prompt约束为自动优先使用快速工具，不会擅自启动VASP计算
...
### ⚛️ 场景五：第一性原理验证（高精度DFT计算）
> **指令**：`"我对MEGNet预测的MAPbI3带隙（1.55 eV）有些怀疑，能否用VASP进行第一性原理计算验证？我的服务器地址是hpc.example.edu，用户名是researcher。"`
> 
> **Agent 行为**：
> 1. 首先尝试用RAG检索文献中的实验值作为参考。
> 2. 用户明确要求DFT计算，Agent将：
>    - 调用 `tool_vasp_connect` 连接远程服务器
>    - 调用 `tool_vasp_prepare_and_submit` 准备输入文件并提交计算（后台运行）
>    - 告知用户"计算已提交到服务器，预计需要4-6小时，请稍后查询"
> 3. 用户稍后回来询问进度时：
>    - 调用 `tool_vasp_check_status` 检查计算状态
>    - 若已完成，调用 `tool_vasp_download_results` 下载结果
>    - 调用 `tool_vasp_parse_bandstructure` 解析能带并报告DFT计算的精确带隙值
> 
> **⚠️ 注意**：此场景展示了Agent如何谨慎使用高成本的VASP工具 - 仅当用户明确要求或需要高精度验证时才启用，且会明确告知计算耗时。
...
**解决方案**：PVmatAgent 在 `electronic_optics.py` 初始化阶段实施了精密的**命名空间级劫持（Monkey Patch）**：
```python
...
│   ├── calculations/
│   │   └── vasp_tools.py         # VASP第一性原理计算专家 (SSH远程提交)
│   ├── database/
